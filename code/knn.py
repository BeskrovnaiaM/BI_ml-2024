    ...
    def predict_labels_binary(self, distances):
        ...
        if self.k > 1:
            ...
            for i in range(n_test):
                matched_classes[i] = self.train_y[indices[i]]
                count_0 = np.sum(matched_classes[i] == 0)
                count_1 = np.sum(matched_classes[i] == 1)
                if count_0 > count_1:
                    num_prediction[i] = 0
                else:
                    num_prediction[i] = 1
            ...
            for i in range(n_test):
                num_prediction[i] = self.train_y[min_distance_index[i]]
            
        prediction = num_prediction.astype(bool)
